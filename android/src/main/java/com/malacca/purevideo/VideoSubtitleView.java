 ...
 *
 * 由于仅用到了 com.google.android.exoplayer:exoplayer-ui 中的 SubtitleView
 * 所以这里仅将该 View 复制过来, 不用再下载整个包了
 */
 ...
 */
class VideoSubtitleView extends View implements TextOutput {
    ...
    private final List<VideoSubtitlePainter> painters;
    ...
    public VideoSubtitleView(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
    ...
    public void setCues(@Nullable List<Cue> cues) {
        ...
        while (painters.size() < cueCount) {
            painters.add(new VideoSubtitlePainter(getContext()));
        }
    ...
    public void setUserDefaultTextSize() {
        setFractionalTextSize(DEFAULT_TEXT_SIZE_FRACTION, false, false);
    }
    ...
    public void setFractionalTextSize(float fractionOfHeight, boolean ignorePadding) {
        setFractionalTextSize(fractionOfHeight, ignorePadding, true);
    }

    // 新增: 支持 ignoreScale 参数
    public void setFractionalTextSize(float fractionOfHeight, boolean ignorePadding, boolean ignoreScale) {
        if (!ignoreScale) {
            float fontScale = Util.SDK_INT >= 19 && !isInEditMode() ? getUserCaptionFontScaleV19() : 1f;
            fractionOfHeight = fontScale * fractionOfHeight;
        }
        setTextSize(
                ignorePadding
     ...
     * Overrides any setting made with {@link VideoSubtitleView#setApplyEmbeddedFontSizes}.
     *
     ...
     * Only takes effect if {@link VideoSubtitleView#setApplyEmbeddedStyles} is set to true.
     *
    ...
    public void dispatchDraw(Canvas canvas) {
        ...
        for (int i = 0; i < cueCount; i++) {
            ...
            VideoSubtitlePainter painter = painters.get(i);
            painter.draw(
                    cue,
                    applyEmbeddedStyles,
